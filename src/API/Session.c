                     ...
                     uint32_t     nameLengthInBytes )
{
   ...
   if ( ! pSession->terminated ) {
      GET_PTR(g_pBaseAddr,  pTree, pSession->pHeader->treeMgrOffset, psonFolder )
                ...
                psoInfo    * pInfo )
{
   ...
   if ( ! pSession->terminated ) {
      ...
      GET_PTR(g_pBaseAddr,  pAlloc, pSession->pHeader->allocatorOffset, psonMemAlloc )
...
//   psocInitErrorHandler( &pSession->context.errorHandler );
   ...
   GET_PTR(g_pBaseAddr,  pSession->context.pAllocator, pSession->pHeader->allocatorOffset, void );
