                         ...
                         psonSessionContext * pContext )
{
   ...
   GET_PTR(pContext->pBaseAddress, pQueueNode, pQueue->nodeOffset, psonTreeNode );
   GET_PTR(pContext->pBaseAddress, pQueueItem, itemOffset, psonQueueItem );
                    ...
                    psonSessionContext * pContext )
{
   ...
   fprintf( pContext->tracefp, "psonQueue (%p) offset = "PSO_PTRDIFF_T_FORMAT"\n",
      pQueue, SET_OFFSET(pContext->pBaseAddress, pQueue) );
   ...
   if ( pQueue->queueType == PSO_LIFO ) {
      DO_INDENT( pContext, indent + 2 );
      fprintf( pContext->tracefp, "Queue type: Last In First out\n" );
   }
   else if ( pQueue->queueType == PSO_QUEUE ) {
      DO_INDENT( pContext, indent + 2 );
      fprintf( pContext->tracefp, "Queue type: First In First Out\n" );
   }
   else {
      DO_INDENT( pContext, indent + 2 );
      fprintf( pContext->tracefp, "Unknown queue type\n" );
   }

   psonLinkedListDump( &pQueue->listOfElements, indent + 2, pContext );

   DO_INDENT( pContext, indent + 2 );
   fprintf( pContext->tracefp, "Data definition offset: "PSO_PTRDIFF_T_FORMAT"\n",
      pQueue->dataDefOffset );

   DO_INDENT( pContext, indent + 2 );
   fprintf( pContext->tracefp, "Number of valid data items: "PSO_SIZE_T_FORMAT"\n",
      pQueue->numValidItems );
   
   psonBlockGroupDump( &pQueue->blockGroup, indent + 2, pContext );

   DO_INDENT( pContext, indent );
   fprintf( pContext->tracefp, "psonQueue END\n" );
}
#endif
                        ...
                        psonSessionContext * pContext )
{
   ...
   GET_PTR(pContext->pBaseAddress, pQueueNode, pQueue->nodeOffset, psonTreeNode );
   GET_PTR(pContext->pBaseAddress, txQueueStatus, pQueueNode->txStatusOffset, psonTxStatus );
                    ...
                    psonSessionContext  * pContext )
{
   ...
   pQueue->nodeOffset = SET_OFFSET(pContext->pBaseAddress, pQueueNode );
   ...
   pQueue->dataDefOffset = SET_OFFSET(pContext->pBaseAddress, ptr);
                      ...
                      psonSessionContext * pContext )
{
   ...
   if ( psonLock( &pQueue->memObject, pContext ) ) {
      ...
      psonLinkNodeInit( &pQueueItem->node, pContext );
      pQueueItem->dataLength = length;
      memcpy( pQueueItem->data, pItem, length );
   
      ok = psonTxAddOps( (psonTx*)pContext->pTransaction,
                         PSON_TX_ADD_DATA,
                         SET_OFFSET(pContext->pBaseAddress, pQueue),
                         PSON_IDENT_QUEUE,
                         SET_OFFSET(pContext->pBaseAddress, pQueueItem),
                         0,
                         pContext );
                         ...
                         psonSessionContext * pContext )
{
   ...
   if ( psonLock( &pQueue->memObject, pContext ) ) {
      ...
      psonTxStatusInit( &pQueueItem->txStatus, PSON_NULL_OFFSET, pContext );
                      ...
                      psonSessionContext * pContext )
{
   ...
   GET_PTR(pContext->pBaseAddress, txParentStatus, pQueueNode->txStatusOffset, psonTxStatus );
   ...
   if ( psonLock( &pQueue->memObject, pContext ) ) {
      ...
      if ( firstOrLast == PSON_QUEUE_FIRST ) {
         ...
         okList = psonLinkedListPeakLast( &pQueue->listOfElements, 
                                          &pNode,
                                          pContext );
      }
      ...
      while ( okList ) {
         ...
         if ( txItemStatus->txOffset == PSON_NULL_OFFSET ) {
            ...
            ok = psonTxAddOps( (psonTx*)pContext->pTransaction,
                               ...
                               SET_OFFSET(pContext->pBaseAddress, pQueue ),
                               PSON_IDENT_QUEUE,
                               SET_OFFSET(pContext->pBaseAddress, pItem ),
                               0, /* irrelevant */
                               pContext );
            PSO_POST_CONDITION( ok == true || ok == false );
            if ( ! ok ) goto the_exit;
            ...
            psonTxStatusMarkAsDestroyed( txItemStatus, pContext );
         ...
         if ( ! psonTxStatusIsRemoveCommitted(txItemStatus, pContext) ) {
            queueIsEmpty = false;
         }
                      ...
                      psonSessionContext * pContext )
{
   ...
   GET_PTR(pContext->pBaseAddress, pQueueNode, pQueue->nodeOffset, psonTreeNode );
   GET_PTR(pContext->pBaseAddress, txStatus, pQueueNode->txStatusOffset, psonTxStatus );
