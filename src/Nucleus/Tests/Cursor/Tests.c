...
void test1( void ** state )
{
   ...
   ok = psonCursorGetFirst( pCursor, &pItem, &context );
   assert_true( ok );
   assert_true( pItem->itemOffset == SET_OFFSET(context.pBaseAddress, (unsigned char *)0x2) );

   ok = psonCursorGetNext( pCursor, pItem, &pItem, &context );
   assert_true( ok );
   assert_true( pItem->itemOffset == SET_OFFSET(context.pBaseAddress, (unsigned char *)0x3) );
   ...
   ok = psonCursorGetPrevious( pCursor, pItem, &pItem, &context );
   assert_true( ok );
   assert_true( pItem->itemOffset == SET_OFFSET(context.pBaseAddress, (unsigned char *)0x1) );
