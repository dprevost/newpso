...
void test_invalid_sig( void ** state )
{
   ...
   list.initialized = 0;
   expect_assert_failure( psonLinkedListGetLast( &list, &pNode, &context ) );
   list.initialized = save;
   psonLinkedListGetLast( &list, &pNode, &context );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonLinkedListGetLast( &list, &pNode, NULL ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonLinkedListGetLast( &list, NULL, &context ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonLinkedListGetLast( NULL, &pNode, &context ) );
#endif
   return;
}
