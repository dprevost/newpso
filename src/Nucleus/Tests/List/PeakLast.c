...
void test_invalid_sig( void ** state )
{
   ...
   list.initialized = 0;
   expect_assert_failure( psonLinkedListPeakLast( &list, &pNode, &context ) );
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonLinkedListPeakLast( &list, &pNode, NULL ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonLinkedListPeakLast( &list, NULL, &context ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonLinkedListPeakLast( NULL, &pNode, &context ) );
#endif
   return;
}
