...
void test_invalid_sig( void ** state )
{
   ...
   list.initialized = 0;
   expect_assert_failure( psonLinkedListPutLast( &list, &node, &context ) );
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonLinkedListPutLast( &list, &node, NULL ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonLinkedListPutLast( &list, NULL, &context ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonLinkedListPutLast( NULL, &node, &context ) );
#endif
   return;
}
